...
type: msp
name: "mcs.matrix~"
summary: "Signal routing and mixing matrix (multichannel I/O)"
signal: true
url: "https://docs.cycling74.com/reference/mcs.matrix~/"
package: "MC"
...
Note: To specify the gain of individual connections, you must use three-element list messages rather than the  connect  message. Connections formed with the  connect  message always have a gain specified by the third argument initially given to the [mcs.matrix~](https://docs.cycling74.com/reference/mcs.matrix~) object. However, subsequent list messages can alter the gain of connections formed with the  connect  message.
...
The [mcs.matrix~](https://docs.cycling74.com/reference/mcs.matrix~) object can receive dictionaries to configure its state. When receiving a dictionary, all previous connections are cleared and replaced by the connections in the dictionary. You can also set some attributes with a dictionary, including the default gain and ramp time.
