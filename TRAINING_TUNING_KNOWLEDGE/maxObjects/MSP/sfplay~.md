...
type: msp
name: "sfplay~"
summary: "Play audio file from disk"
signal: true
url: "https://docs.cycling74.com/reference/sfplay~/"
package: "MSP"
see_also: ["buffer~", "groove~", "mc.sfplay~", "play~", "sfinfo~", "sflist~", "sfrecord~", "mc.sfrecord~"]
---
# sfplay~
...
Use the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object to play audio files from disk. Supported formats include AIFF, WAVE, MP3, OGG, FLAC, M4A, CAF, WF64, WAVE64, and Raw Data.

#### Discussion

To play a file, you send [sfplay~](https://docs.cycling74.com/reference/sfplay~) the  open  message, then send it a  1  to start and a  0  to stop.  open  takes an argument to specify a filename in the search path. You can also create additional cues with the  preload  message. These can reference other files, all of which are simultaneously accessible. The  open  message sets the "current" file: the one that plays back from the beginning when  1  is sent and is used as the default for the  preload  message. [sfplay~](https://docs.cycling74.com/reference/sfplay~) can also connect to the cues defined in an [sflist~](https://docs.cycling74.com/reference/sflist~) object. Since multiple [sfplay~](https://docs.cycling74.com/reference/sfplay~) objects can reference the same [sflist~](https://docs.cycling74.com/reference/sflist~), this allows you to store a global list of cues.

## Arguments

### sflist-object-name[symbol] optional

If the first argument is a  symbol , it names an [sflist~](https://docs.cycling74.com/reference/sflist~) that the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object will use for playing cues. If no symbol argument is given, [sfplay~](https://docs.cycling74.com/reference/sfplay~) plays its own internally defined cues.

### number-of-output-channels[int] optional

Sets the number of output channels, which determines the number of signal outlets that the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object will have. The maximum number of channels is 64. The default is 1. If the audio file being played has more output channels than the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object, higher-numbered channels will not be played. If the audio file has fewer channels, the signals coming from the extra outlets of [sfplay~](https://docs.cycling74.com/reference/sfplay~) will be 0.
...
If the last argument is a symbol, it specifies a name by which other objects can refer to the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object to access its contents.

## Attributes

### audiofile[symbol] >= 7.0.0

Specifies the name/pathname of an audio file to be loaded by the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object.
...
When followgobaltempo is enabled, [sfplay~](https://docs.cycling74.com/reference/sfplay~) will calculate the current tempo out of the ratio between originaltempo and global tempo and adapt to global tempo changes.
...
In left inlet: Gives a set of cues for [sfplay~](https://docs.cycling74.com/reference/sfplay~) to play, one after the other. The maximum number of cues is in a list is 128. Cue numbers (set using the  preload  message) can be any integer greater than or equal to 2.If a cue number in a list has not been defined, it is skipped and the next cue, if any, is tried. If the object is currently assigned to an [sflist~](https://docs.cycling74.com/reference/sflist~) object, a list uses cues stored in the [sflist~](https://docs.cycling74.com/reference/sflist~) object. Otherwise, cues stored inside the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object are used.
...
In left inlet: If the name of an [sflist~](https://docs.cycling74.com/reference/sflist~) object is sent to [sfplay~](https://docs.cycling74.com/reference/sfplay~), followed by a number, the numbered cue from the [sflist~](https://docs.cycling74.com/reference/sflist~) is played if it exists.
...
When an audio file is dragged from the Max File Browser to an [sfplay~](https://docs.cycling74.com/reference/sfplay~) object, the file will be loaded.

### clear

In left inlet: The word  clear  with no arguments clears all defined cues. After a  clear  message is received, only the number  1  will play anything (assuming there's an open file). The word  clear  followed by one or more cue numbers removes them from the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object's cue list.  clear  requires the DSP chain to be off.
...
In left inlet: The message  embed , followed by any non-zero integer, causes [sfplay~](https://docs.cycling74.com/reference/sfplay~) to save all of its defined cues and the name of the current open file when the patcher file is saved. The message  embed   0  keeps [sfplay~](https://docs.cycling74.com/reference/sfplay~) from saving this information when the patcher is saved. By default, the current file name and the cue information is not saved in [sfplay~](https://docs.cycling74.com/reference/sfplay~) when the patcher is saved. If an [sfplay~](https://docs.cycling74.com/reference/sfplay~) object is saved with stored cues, they will all be preloaded when the patcher containing the object is loaded.
...
In left inlet: followed by the name of an AIFF, WAVE, MP3, OGG, FLAC, M4A, CAF, WF64, WAVE64, or Raw format audio file or CD-audio track, opens the file for playback and makes it the current file. The word  open , followed by a filename, opens the file if it exists in Max's search path. Without a filename,  open  brings up a standard open file dialog allowing you to choose a file. When a file is opened, its beginning is read into memory, and until another file is opened, you can play the file from the beginning by sending [sfplay~](https://docs.cycling74.com/reference/sfplay~) the message  1 . When the  open  message is received, the previous current file, if any, remains open and can be referred to by name when defining a cue with the  preload  message. If any cues were defined that used the previous current file, they are still valid even if the file is no longer current.
...
In left inlet: Defines a cue - an integer greater than or equal to 2 - to refer to a specific region of a file. When that cue number is subsequently received, [sfplay~](https://docs.cycling74.com/reference/sfplay~) plays that region of that file. Cue number 1 is always the beginning of the current file - the file last opened with the  open  message - and cannot be modified with the  preload  message.
...
A final optional argument is used to set the playback speed. A float value sets the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object's playback speed relative to the object's global playback speed -- set by either the  speed  message or the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object's right inlet. The default value is  1 .
...
The  preloadn  message works in precisely the same manner as the  preload  message, but normalized values in the range 0. - 1.0 are to specify start and end times for the cue.
...
In left inlet: The message  set , followed by a name of an [sflist~](https://docs.cycling74.com/reference/sflist~) object, will cause [sfplay~](https://docs.cycling74.com/reference/sfplay~) to play cues stored in the [sflist~](https://docs.cycling74.com/reference/sflist~) when it receives an  int  or  list . The message  set  with no arguments resets [sfplay~](https://docs.cycling74.com/reference/sfplay~) to use its own internally defined cues when receiving an  int  or  list .
...
There is one signal outlet for each of the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object's specified output channels (set by or as an argument to the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object) that sends out the audio data of the corresponding channel of the audio file when a cue number is received in the inlet. (The left outlet plays channel 1, and so on.)

If the optional output position argument is specified, there will be one or two signal outputs following the channel outputs whose signal outputs display positioning information. If the argument is  1 , a single outlet to the left of the rightmost "bang on finish or halt" outputs a signal containing the current playback position in milliseconds. Specifying a final argument of  2  creates a second outlet which outputs a signal containing the playback position single precision roundoff error in milliseconds (see Arguments for a more detailed description of the [sfplay~](https://docs.cycling74.com/reference/sfplay~) object's position outlets).
