...
signal: true
url: "https://docs.cycling74.com/reference/mc.tapout~/"
package: "MC"
see_also: ["delay~", "tapin~", "tapout~", "mc.tapin~"]
---
# mc.tapout~
...
The outlet of an [mc.tapin~](https://docs.cycling74.com/reference/mc.tapin~) object must be connected to the left inlet of [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) in order for the delay line to function.

The [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) object has one or more inlets and one or more outlets. A delay time signal or number received in an inlet affects the output signal coming out of the outlet directly below the inlet.

## Arguments

### initial-delay[number]ms optional

One or more initial delay times in milliseconds, one for each delay "tap" inlet-outlet pair desired. For example, the arguments  50 100 300  would create an [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) object with three independent "taps" corresponding to three inlets and three outlets. If a  signal  is connected to an inlet, the initial delay time corresponding to that inlet-outlet pair is ignored. The minimum delay time that can be used is determined by the current signal vector size (Options->Audio Status). If the Signal Vector Size is set to 64, any delay less than 64 samples will be limited to a minimum of 64 samples.
...
If a  signal  is connected to an inlet of [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~), the signal coming out of the outlet below it will use a continuous delay algorithm. Incoming signal values represent the delay time in milliseconds. If the signal increases slowly enough, the pitch of the output will decrease, while, if the signal decreases slowly, the pitch of the output will increase. The continuous delay algorithm is more computationally expensive than the fixed delay algorithm that is used when a signal is not connected to an [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) inlet.

### tapconnect

The word  tapconnect  will connect an [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) object to an available [mc.tapin~](https://docs.cycling74.com/reference/mc.tapin~) object remotely.

## Output

### signal

Each outlet of [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) corresponds to an individually controlled "tap" of a delay line written by the [mc.tapin~](https://docs.cycling74.com/reference/mc.tapin~) object. The output signal coming out of an [mc.tapout~](https://docs.cycling74.com/reference/mc.tapout~) outlet is the input to [mc.tapin~](https://docs.cycling74.com/reference/mc.tapin~) delayed by the number of milliseconds specified by the numerical or signal control received in the inlet directly above the outlet.
...
| [mc.tapin~](https://docs.cycling74.com/reference/mc.tapin~) | Input to a delay line |
