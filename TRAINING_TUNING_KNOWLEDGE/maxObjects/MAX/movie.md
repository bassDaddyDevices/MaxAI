...
type: max
name: "movie"
summary: "Play a movie in a window"
signal: false
url: "https://docs.cycling74.com/reference/movie/"
package: "Max"
see_also: ["imovie"]
---
# movie
...
Note: The [movie](https://docs.cycling74.com/reference/movie) object requires that QuickTime be installed on your system. If you are using Max on Windows, we recommend that you install QuickTime and choose a complete install of all optional components.The [movie](https://docs.cycling74.com/reference/movie) object plays a movie in its own window, and the [imovie](https://docs.cycling74.com/reference/imovie) object plays a movie in a box inside a patcher window.

## Arguments

### filename[symbol] optional

Specifies the name of a movie file to be read into [movie](https://docs.cycling74.com/reference/movie) automatically when the patch is loaded. The same effect can be achieved for [imovie](https://docs.cycling74.com/reference/imovie) by selecting the object in an unlocked patcher and choosing Get Info... from the Object menu to select a movie file. Both objects retain the name(s) of the movie(s) they have loaded at the time that the patch is saved, and attempt to load the same movie(s) the next time the patch is opened.
...
When a QuickTIme movie file is dragged from the Max File Browser to a [movie](https://docs.cycling74.com/reference/movie) object, the file will be loaded.
...
Double-clicking on the [movie](https://docs.cycling74.com/reference/movie) object will make the movie window active.

### dispose

Closes the movie window if it is open, and removes all movies from the [movie](https://docs.cycling74.com/reference/movie) object's memory. If the word  dispose  is followed by the name of a loaded movie, only the named movie will be removed.
...
Brings the movie window to the foreground (applies only to [movie](https://docs.cycling74.com/reference/movie), not [imovie](https://docs.cycling74.com/reference/imovie)).
...
The word  windowpos , followed by four numbers, specifies the location and size of the movie window on the screen. The four numbers specify the left, top, right, and bottom of the movie window in global coordinates. This message is only supported by the [movie](https://docs.cycling74.com/reference/movie) object, not the [imovie](https://docs.cycling74.com/reference/imovie) object.
