...
type: max
name: "string.append"
summary: "Append a string to another string object, with an optional separator"
signal: false
url: "https://docs.cycling74.com/reference/string.append/"
package: "Max"
see_also: ["string", "string.concat", "string.prepend"]
---
# string.append

Append a string to another string object, with an optional separator

## Arguments

### initial-string[list] optional

An initial string to append to incoming strings
...
In the left inlet, the append string will be applied to an incoming string, as well as any separator placed between them, and the result will be sent to the outlet. In the middle inlet, a string can be set to be appended to another string without triggering output. In the right inlet, an optional string separator between the string and append string can be set without triggering output.
