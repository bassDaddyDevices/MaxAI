...
type: msp
name: "mcs.groove~"
summary: "Variable-rate looping sample playback"
signal: true
url: "https://docs.cycling74.com/reference/mcs.groove~/"
package: "MC"
see_also: ["2d.wave~", "buffer~", "groove~", "mc.groove~", "play~", "wave~", "index~", "record~", "transport"]
---
# mcs.groove~
...
The interpolation for mcs.groove~ is cubic unless the timestretching attribute is set to 1. When timestretching is set to 1, you can control the quality of the conversion with the mode, quality, and formant attributes. The [wave~](https://docs.cycling74.com/reference/wave~) object provides additional interpolation options for buffer playback.

## Arguments

### buffer-name[symbol] optional

Names the [buffer~](https://docs.cycling74.com/reference/buffer~) object containing the sample to be used by [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) for playback.

### number-of-outputs[int] optional

A second argument may specify the number of output channels. The default number of channels is 1. If the [buffer~](https://docs.cycling74.com/reference/buffer~) being played has fewer channels than the number of [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) output channels, the extra channels output a zero signal. If the [buffer~](https://docs.cycling74.com/reference/buffer~) has more channels and is a multiple of 2 or 4, channels are mixed. For [groove~](https://docs.cycling74.com/reference/groove~) and [mc.groove~](https://docs.cycling74.com/reference/mc.groove~), channels and sent out of separate outlets. For [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~), all channels are output from the first outlet as a multi-channel signal.
...
When followgobaltempo is enabled, [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) will calculate the current tempo out of the ratio between originaltempo and global tempo and adapt to global tempo changes.
...
Sets the name of a [transport](https://docs.cycling74.com/reference/transport) object with which to associate. By default, the global transport is used. When the [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) object is associated with a transport, loop points are specified using 3-item lists which correspond to time in bars, beats, and units.
...
In middle inlet: A list composed of three integers specifying bars, beats, and units may be used to specify the loop start point when the [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) object is set to follow a named transport (set using the  transport  attribute).

In right inlet: A list composed of three integers specifying bars, beats, and units may be used to specify the loop end point when the [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) object is set to follow a named transport (set using the  transport  attribute).
...
Double-clicking on an [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) object opens the sample display window of the [buffer~](https://docs.cycling74.com/reference/buffer~) object associated with the [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) object.
...
The word  set , followed by a symbol, switches the [buffer~](https://docs.cycling74.com/reference/buffer~) object containing the sample to be used by [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) for playback.
...
Causes [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) to begin sample playback at the starting point of the loop. If no loop has been defined, [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) begins playing at the beginning.

### stop

The word  stop  will cause [mcs.groove~](https://docs.cycling74.com/reference/mcs.groove~) to stop playback until the next  int ,  float , or  startloop  message is received.
...
| [mc.groove~](https://docs.cycling74.com/reference/mc.groove~) | Variable-rate looping sample playback (multichannel) |
